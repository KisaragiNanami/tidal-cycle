...
export async function POST({ request }: APIContext): Promise<Response> {
  try {
    ...
    const SENSITIVE_USERS = ["博主", "kisaragi.nanami@qq.com"];
    ...
    if (!secretPassword) {
      return new Response(
        JSON.stringify({ success: false, message: "服务器未配置密码。" }),
        { status: 500 },
      );
    }
    ...
    if (password === secretPassword) {
      ...
      const response = new Response(
        JSON.stringify({ success: true, isAdmin: true, message: "验证成功" }),
        {
          status: 200,
    ...
    // 密码错误
    return new Response(
      JSON.stringify({ success: false, message: "密码不正确" }),
      { status: 401 },
    );
  } catch (error) {
    console.error(error);
